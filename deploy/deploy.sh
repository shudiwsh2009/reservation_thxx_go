...
cp -f $GOPATH/bin/server ./server.run
...
nohup ./server.run --app-env="ONLINE" --sms-uid="shudiwsh2009" --sms-key="946fee2e7ad699b065f1" > log/server.log 2>&1 & echo $! > ~/thxxfzzx_go/run.pid &

echo "#deploy reminder"
cd ~/thxxfzzx_go
go install github.com/shudiwsh2009/reservation_thxx_go/reminder
cp -f $GOPATH/bin/reminder ./reminder.run
